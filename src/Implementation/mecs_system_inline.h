namespace mecs::system
{
    ...
    namespace details
    {
                                ...
                                ,   const archetype::query::result_entry& R ) noexcept
        {
            ...
            const auto  Decide      = [&]( int iArg, auto Arg ) constexpr noexcept
            {
                ...
                if( I.m_isShared ) return reinterpret_cast<std::byte*>(&R.m_pArchetype->m_PoolList.m_EntityPool.getComponentByIndex<t>( Index, I.m_Index ));

                auto& Specialized = R.m_pArchetype->m_PoolList.m_EntityPool.getComponentByIndex<mecs::archetype::pool>(Index, 0);
                                                ...
                                                ,  const int                            iEnd ) noexcept
        {
            ...
            {
                ShareComponentData[i] = Pool.m_pArchetypeInstance->m_PoolList.m_EntityPool.getComponentByIndexRaw( Pool.m_MainPoolIndex, 1+i );
    ...
    void instance::ForEach(mecs::archetype::query::instance& QueryI, T_CALLBACK&& Functor, int nEntitiesPerJob ) noexcept
    {
        ...
        {
            auto& MainPool = R.m_pArchetype->m_PoolList.m_EntityPool;
    ...
    void instance::_ProcessResult( T_PARAMS& Params, T_PARAMS2& Params2, const int Index ) noexcept
    {
        ...
        auto&               SpecializedPool     = Params2.m_pResult->m_pArchetype->m_PoolList.m_EntityPool.getComponentByIndex<mecs::archetype::pool>(Index, 0);
        ...
        if constexpr ( std::tuple_size_v<mutable_share_tuple> > 0 )
        {
            for (int end = static_cast<int>(SpecializedPool.m_EntityPool.size()), i = 0; i < end; )
            {
                ...
                ] () constexpr noexcept
                {
                    ...
                    auto&       SpecializedPool         = Params2.m_pResult->m_pArchetype->m_MainPool.getComponentByIndex<mecs::archetype::pool>(Index, 0);
