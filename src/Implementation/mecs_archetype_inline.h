namespace mecs::archetype
{
    namespace details
    {
        ...
        struct share_component_get< std::tuple<T_TUPLE_ARGS...> >
        {
            ...
            static auto Call( pool& Pool, T_CALLBACK&& Callback ) noexcept
            {
                ...
                for( int i=0, end = static_cast<int>(DescriptorSpan.size()); i<end; ++i )
                {
                    ...
                    {
                        i++;
                        SortedPointers[j] = Archetype.m_PoolList.m_EntityPool.getComponentByIndexRaw( Pool.m_MainPoolIndex, i );
                        j++;
                    }
                    else
                    {
    ...
    void pool::getShareComponent( T_CALLBACK&& CallBack ) noexcept
    {
    ...
    ,   mecs::component::entity::map*                               pEntityMap          ) noexcept
    {
        ...
        m_MainPoolDescriptorArrayData[0] = &mecs::component::descriptor_v<pool>;
    ...
    void instance::MemoryBarrierSync( mecs::sync_point::instance& SyncPoint ) noexcept
    {
        ...
        //
        m_PoolList.m_EntityPool.MemoryBarrier();
        if(m_PoolList.m_EntityPool.size()==0) return;

        for( int i=0, end = static_cast<int>(m_PoolList.m_EntityPool.size()); i != end; ++i )
        {
            auto& Specialized = m_PoolList.m_EntityPool.getComponentByIndex<pool>(i,0);
    ...
    void instance::Start( void ) noexcept
    {
        ...
        //
        for (int end = m_PoolList.m_EntityPool.size(), i = 0; i < end; ++i)
        {
            auto& Specialized = m_PoolList.m_EntityPool.getComponentByIndex<pool>(i, 0);
            Specialized.m_EntityPool.MemoryBarrier();
        }
    ...
    ,   T_SHARE_COMPONENTS&&...                     ShareComponents ) noexcept
    {
        ...
        if constexpr ( !!(sizeof...(T_SHARE_COMPONENTS)) )
        {
            ...
            const pool::type_guid SpecializedGuidTypeGuid{ (ShareComponentKeys[xcore::types::tuple_t2i_v<T_SHARE_COMPONENTS, share_tuple>] + ...) };

            static constexpr auto nKeys = sizeof...(T_SHARE_COMPONENTS);
            for (int end = static_cast<int>(m_PoolList.m_EntityPool.size()), i = 0; i < end; ++i)
            {
                auto& Specialized = m_PoolList.m_EntityPool.getComponentByIndex<pool>(i, 0);
            ...
            const int PoolIndex   = m_PoolList.m_EntityPool.append(); m_PoolList.m_EntityPool.MemoryBarrier();
            auto&     Specialized = m_PoolList.m_EntityPool.getComponentByIndex<pool>(PoolIndex, 0);
            ...
            ( 
                (
                    m_PoolList.m_EntityPool.getComponentByIndex<T_SHARE_COMPONENTS>
                    (
                        PoolIndex
    ...
    pool::type_guid instance::getSpecializedPoolGuid( const instance& Instance, T_SHARE_COMPONENTS&&...ShareComponents ) const noexcept
    {
        static_assert(mecs::component::descriptor_v<T_SHARE_COMPONENTS>.m_Type == mecs::component::type::SHARE);
    ...
    TRY_FIND_AGAIN:
        ...
        for( int end = m_PoolList.m_EntityPool.size(); i<end; ++i )
        {
            auto& Specialized = m_PoolList.m_EntityPool.getComponentByIndex<pool>(i, 0);
        ...
        if( pPool == nullptr )
        {
            ...
            if( i != m_PoolList.m_EntityPool.size() ) goto TRY_FIND_AGAIN;
            ...
            for( int i=0; i< Specialized.m_ShareComponentKeysSpan.size(); i++ )
            {
                ...
                memcpy(m_PoolList.m_EntityPool.getComponentByIndexRaw(PoolIndex, 1 + i), pPointersToShareComponents[i], m_PoolDescriptors.m_ShareDescriptor[i]->m_Size );
            }
        ...
        auto& OldSpecialized = m_PoolList.m_EntityPool.getComponentByIndex<pool>(FromSpecializePoolIndex, 0);
    ...
    template< typename... T_SHARE_COMPONENTS >
    inline
    pool& instance::getOrCreateSpecializedPool( system::instance&         System
                                                          , int                       MinFreeEntries
                                                          , int                       MaxEntries    
                                                          ...
                                                          ) noexcept
    {
        ...
        CONTINUE_SEARCHING:
        {
            ...
            for (int end = m_PoolList.m_EntityPool.size(); i < end; ++i)
            {
                auto& Pool = m_PoolList.m_EntityPool.getComponentByIndex<pool>(i, 0);
        ...
        {
            xcore::lock::scope Lk(m_SemaphoreLock);
            if (i != m_PoolList.m_EntityPool.size()) goto CONTINUE_SEARCHING;

            const auto Index = m_PoolList.m_EntityPool.append(1);
            auto&      Pool  = m_PoolList.m_EntityPool.getComponentByIndex<pool>( Index, 0 );
            ...
            ((m_PoolList.m_EntityPool.getComponentByIndex<T_SHARE_COMPONENTS>(Index, 1 + xcore::types::tuple_t2i_v< T_SHARE_COMPONENTS, sorted_tuple> ) = ShareComponents), ... );
            ...
            m_PoolList.m_EntityPool.UpdateCount();
    ...
    */
    ...
    namespace details
    {
        ...
        //---------------------------------------------------------------------------------
        template< typename T_CALLBACK, typename...T_ARGS, typename...T_EXTRA_ARGS > xforceinline
        void CallPoolFunction( mecs::archetype::pool& Pool, T_CALLBACK&& Callback, std::tuple<T_ARGS...>*, T_EXTRA_ARGS&... ExtraArgs ) noexcept
        {
            static_assert(( (mecs::component::descriptor_v< xcore::types::decay_full_t<T_ARGS> >.m_Type == mecs::component::type::SHARE) && ... ));
                ...
                , ([&]() constexpr noexcept -> T_ARGS
                    {
                        using arg_t = xcore::types::decay_full_t<T_ARGS>;
                        {
                            auto& C = Archetype.m_PoolList.m_EntityPool.getComponentByIndex<arg_t>( Pool.m_MainPoolIndex, Indices[xcore::types::tuple_t2i_v<T_ARGS, std::tuple<T_ARGS...>>]);
                            if constexpr (std::is_pointer_v<T_ARGS>) return reinterpret_cast<T_ARGS>(&C);
                            else                                     return reinterpret_cast<T_ARGS>(C);
                        }
                    }())...
    ...
    */
      ...
      , T_SHARE_COMPONENTS&&...     ShareComponents ) noexcept
    {
        ...
        pool*   pSpecialized = nullptr;
        if(!!sizeof...(T_SHARE_COMPONENTS))
        {
            auto& MainPool = NewArchetype.m_PoolList.m_EntityPool;
            auto  FindPool = [&]
            {
                for (int i = 0, end = static_cast<int>(NewArchetype.m_PoolList.m_EntityPool.size()); i != end; ++i)
                {
                    auto& Specialized = MainPool.getComponentByIndex<pool>(i,0);
            ...
            if(pSpecialized == nullptr)
            {
                ...
                if (pSpecialized == nullptr)
                {
                    ...
                    auto& Specialized = MainPool.getComponentByIndex<pool>(Index, 0);
